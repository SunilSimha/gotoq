...
class CustomViewBox(pg.ViewBox):
	"""ViewBox where plain scroll zooms X only and Ctrl+scroll zooms Y only."""
...
class QSOViewer(QtWidgets.QMainWindow):
	def __init__(self, store: QSODataStore):
		...
		self._fine_scale = 100000.0
		self._keyboard_redshift_step_multiplier = 100
		self._keyboard_redshift_coarse_multiplier = 1000
	...
	def _shift_error_clip(self, spectrum: Dict[str, np.ndarray]) -> np.ndarray:
		"""Return error_clip shifted so its median equals 1 - 3*std(flux_clip)."""
